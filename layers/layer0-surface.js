 ...
 * Адаптировано под азимутальную проекцию (x,z) и движение по часовой стрелке
 * @version 6.0.1
 */
...
class SurfaceLayer {
    ...
    createLightingOverlay() {
        ...
        const material = new THREE.ShaderMaterial({
            ...
            depthWrite: false,
            uniforms: {
                sunPosition:   { value: new THREE.Vector2(0, 0) }, // (x,z) на плоскости
                moonPosition:  { value: new THREE.Vector2(0, 0) }, // (x,z) на плоскости
            ...
            fragmentShader: `
                ...
                void main() {
                    ...
                    // Расчёт "затмения": когда Луна близко к Солнцу, солнечный свет частично гасится
                    ...
                    // Добавляем голубоватое лунное пятно
                    vec3 moonTint = vec3(0.6, 0.7, 0.9);
                    color = mix(color, moonTint, moonLight * 0.4);

                    // Затемнение: днём почти прозрачно, ночью темно
    ...
    updateLighting(sunPos, moonPos) {
        // ВАЖНО: используем (x,z) как проекцию на плоскость
        ...
        if (this.lightingOverlay && this.lightingOverlay.material.uniforms) {
            // Обновление позиций в шейдере (x,z)
